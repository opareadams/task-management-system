...
class MemberAllTimeLogController extends MemberBaseController
{
    ...
    public function data($startDate = null, $endDate = null, $projectId = null) {
        ...
        if(!$this->user->can('view_timelogs')){
            $timeLogs = $timeLogs->join('project_members', 'project_members.project_id', '=', 'projects.id');
        }

        $timeLogs->select('project_time_logs.id', 'projects.project_name', 'project_time_logs.start_time', 'project_time_logs.end_time', 'project_time_logs.total_hours', 'project_time_logs.total_minutes', 'project_time_logs.memo', 'project_time_logs.project_id', 'project_time_logs.user_id');
        ...
        if(!$this->user->can('view_timelogs')){
            $timeLogs = $timeLogs->where('project_members.user_id', '=', $this->user->id);
        }
